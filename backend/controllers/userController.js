...
export const updateUserDescription = async (req, res) => {
    try {
        ...
        if (!userdescription) {
            return res.status(400).json({ message: "User description cannot be empty", success: false });
        }
        ...
        user.userdescription = userdescription; // Update the description
        await user.save();
...
export const updateUserImages = async (req, res) => {
    try {
        const { profileImage, bannerImage, id } = req.body;
